...
type Server struct {
	...
	stopFlag   int32
	stopWait   *sync.WaitGroup // [Raven] to make all sessions close elegantly when server stoping
...
func (server *Server) Accept() (*Session, error) {
	...
	session := server.newSession( // [Raven] newSession invoke session.server.putSession(session)
...
func (server *Server) Stop(reason interface{}) {
	if atomic.CompareAndSwapInt32(&server.stopFlag, 0, 1) {
		...
		server.stopWait.Wait() // [Raven] wait for all sessions to close
	}
}
...
func (server *Server) closeSessions() {
	...
	for _, session := range server.sessions { // [Raven] why copy all the sessions to close
